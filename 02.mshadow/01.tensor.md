...
* 提供FlatTo1D/FlatTo2D/取下标/Slice等操作；
```
 ...
 struct Tensor: public TRValue<Tensor<Device, dimension, DType>,
  ...
  public:
   ...
   index_t stride_; // 默认为最低维度大小，此时，内存连续存储；
   ...
   MSHADOW_XINLINE bool CheckContiguous(void) const { return this->shape_[dimension - 1] == stride_; }
