  ...
  public:
   ...
   inline mshadow::Tensor<Device, dim, DType> get(mshadow::Stream<Device> *stream = NULL) const {
     ...
     return mshadow::Tensor<Device, dim, DType>(dptr<DType>(),
         shape_.get<dim>(), // mxnet::TShape转换为mshadow::Shape<dim>；dim必须等于shape_.ndim()，否则会报错；
   ...
   template<typename Device, typename DType>
   inline mshadow::Tensor<Device, 3, DType> FlatTo3D(int axis, mshadow::Stream<Device> *stream = NULL) const;
   template<typename Device, typename DType>
   ...
   /* 升维：高维度补1；降维：高维度合并； */
   template<typename Device, int dim, typename DType>
   inline mshadow::Tensor<Device, dim, DType> FlatToKD(mshadow::Stream<Device> *stream = NULL) const {
