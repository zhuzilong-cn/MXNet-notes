...
```c++
 class ThreadedVar final
  ...
  private:
   ...
   VersionedVarBlock* pending_write_{nullptr}; // 当前正在处理的write op
  ...
  public:
   ...
   inline void AppendReadDependency(OprBlock* opr_block) {
     ...
     if (pending_write_ == nullptr) { // 没有正在处理的write op
   ...
   inline void AppendWriteDependency(OprBlock* opr_block) {
     ...
     if (pending_write_ == nullptr) { // 当前无pending的write op
       pending_write_ = head_; // 当前添加的op即为最新的write op
   ...
   inline void CompleteReadDependency(Dispatcher dispatcher) {
     ...
     {
       std::lock_guard<std::mutex> lock{mutex_};
       CHECK_GT(num_pending_reads_, 0); // 至少有一个read op
   ...
   inline bool CompleteWriteDependency(Dispatcher dispatcher) {
     ...
     { // 加锁访问
       ...
       while (end_of_read_chain != head_ && // 是否到达队尾
              end_of_read_chain->write == false) { // 读操作
         ++num_pending_reads_; // 递增read op数
