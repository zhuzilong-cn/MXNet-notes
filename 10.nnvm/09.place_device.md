...
* 正向(后序)遍历图，用节点的__ctx_group__属性从device_assign_map获取设备id配置；
* 正向(后序)遍历图，当op为forward节点且未配置设备id时，将其inputs中第一个配置的设备id(如果有)设置为该op的设备id；
* 正向(后序)遍历图，当op为backward节点且未配置设备id时，将其forward节点的设备id设置为该op的设备id；
