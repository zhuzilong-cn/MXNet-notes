...
实现NextChunk/NextRecord等函数，引入溢出缓冲区，确保读入的每块chunk不包含不完整的record；
子类需要实现ExtractNextRecord从chunk抽取具体的record，实现SeekRecordBegin/FindLastRecordBegin以便定位record边界，确保每个分片/chunk边界明确、不会把一个record分到两个分片或两个chunk；子类还需要实现IsTextParser，便于处理文本文件结束无换行符的case；
```
