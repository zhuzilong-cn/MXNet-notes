...
```
## FieldEntry
对struct中的字段进行抽象，支持指定参数名、类型、描述、默认值、取值范围等，支持按统一方式对字段进行赋值(Set函数)、获取(GetStringValue)及合规性检查(Check函数)；
```
...
```
## ParamManager
对某个struct的所有字段进行统一管理，支持字段别名设置，支持通过kv方式(name=value)对字段进行赋值及合规检查，支持获取字段kv字典；
```
 class ParamManager {
  private:
   std::string name_;                                   // struct结构体名字
   std::vector<FieldAccessEntry*> entry_;               // struct字段列表
   std::map<std::string, FieldAccessEntry*> entry_map_; // 字段名到FieldEntry的映射字典

  public:
   inline void set_name(const std::string &name); // 设置struct名
   
   inline void AddEntry(const std::string &key, FieldAccessEntry *e);        // 添加struct字段
   ...
   /* 根据kv字典设置struct字段值，并进行合规检查 */
   template<typename RandomAccessIterator>
   inline void RunInit(void *head, // struct指针
...
···
 ...
 struct ParamManagerSingleton {
   ...
   explicit ParamManagerSingleton(const std::string &param_name) {
     PType param;                  // Parameter实例，用于初始化该struct对应的ParamManager
     manager.set_name(param_name); // 设置struct名字
